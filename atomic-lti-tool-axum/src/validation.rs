...
impl LtiLaunchValidator {
  ...
  pub async fn validate_launch(
    ...
    Self::validate_basic_oidc(id_token, oidc_state_store).await?;
  ...
  async fn validate_basic_oidc(
    ...
    let created_at = oidc_state_store.get_created_at().await;
    ...
    if (now - iat).num_minutes().abs() > 5 {
      return Err(ToolError::Unauthorized(
        "Token issued at invalid time".to_string(),
      ));
    }
  ...
  async fn validate_issuer(
    ...
    let _platform_oidc_url = platform_store
      .get_oidc_url()
      .await
  ...
  fn validate_target_link_uri(token_target: &str, expected_target: &str) -> Result<(), ToolError> {
  ...
  fn validate_deployment_id(deployment_id: &str) -> Result<(), ToolError> {
    if deployment_id.is_empty() {
      return Err(ToolError::BadRequest("Missing deployment_id".to_string()));
    }
  ...
  fn validate_resource_link(id_token: &IdToken) -> Result<(), ToolError> {
    if let Some(resource_link) = &id_token.resource_link {
      if resource_link.id.is_empty() {
        return Err(ToolError::BadRequest("Empty resource link ID".to_string()));
      }
    } else {
  ...
  fn validate_roles(roles: &[String]) -> Result<(), ToolError> {
    ...
    for role in roles {
      let is_valid = VALID_ROLE_PREFIXES
        .iter()

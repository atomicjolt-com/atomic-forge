...
mod tests {
    ...
    fn test_append_query_params() {
        ...
        assert_eq!(result, "https://example.com/path?existing=param&new_param=new_value");
    }

    #[test]
    fn test_extract_domain() {
        assert_eq!(extract_domain("https://example.com/path"), Some("example.com".to_string()));
        assert_eq!(extract_domain("https://sub.example.com/path"), Some("sub.example.com".to_string()));
    ...
    fn test_validate_redirect_url() {
        ...
        assert!(validate_redirect_url("https://example.com/path", &allowed_domains));
        assert!(validate_redirect_url("https://sub.example.com/path", &allowed_domains));
        assert!(validate_redirect_url("https://trusted.org/path", &allowed_domains));
        assert!(!validate_redirect_url("https://evil.com/path", &allowed_domains));
        assert!(!validate_redirect_url("http://example.com/path", &allowed_domains));
    }

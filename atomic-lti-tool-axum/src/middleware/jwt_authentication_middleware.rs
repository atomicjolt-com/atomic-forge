...
// Basic claims for backward compatibility
...
// Extended LTI claims that include launch context
#[derive(Debug, Serialize, Deserialize, Clone)]
  ...
  T: LtiDependencies + Send + Sync + 'static,
{
  ...
  let rsa_key = match key_store.get_key(&kid).await {
    Ok(key) => key,
    Err(_) => {
  ...
  let public_key_pem = rsa_key.public_key_to_pem().map_err(|_| {
    (
  ...
  // Try to decode as LtiClaims first, fall back to basic Claims
  match decode::<LtiClaims>(token, &decoding_key, &validation) {
    Ok(token_data) => {
      // Add LTI claims to request extensions for downstream handlers
      request.extensions_mut().insert(token_data.claims);
      Ok(next.run(request).await)
    }
    Err(_) => {
      // Fall back to basic Claims for compatibility

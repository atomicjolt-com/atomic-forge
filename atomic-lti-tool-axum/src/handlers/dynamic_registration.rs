  ...
  T: LtiDependencies + Send + Sync + 'static,
{
  ...
  let registration_endpoint = openid_config["registration_endpoint"]
    .as_str()
    .ok_or_else(|| {
      ToolError::BadRequest("Missing registration_endpoint in OpenID config".to_string())
    })?;
...
<body>
    ...
    <p>You can now use this tool in your LTI platform. Make sure to save the client ID and other configuration details for future reference.</p>
        ...
        "#,
    client_id = registration_response
      .get("client_id")
      .and_then(|v| v.as_str())
      .unwrap_or("unknown"),
    ...
    registration_response =
      serde_json::to_string_pretty(&registration_response).unwrap_or_default()
  );

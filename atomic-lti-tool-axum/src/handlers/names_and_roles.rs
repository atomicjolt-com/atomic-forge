  ...
  T: LtiDependencies + Send + Sync + 'static,
{
  ...
  let nrps_endpoint = claims.names_and_roles_endpoint_url.ok_or_else(|| {
    ToolError::BadRequest("Names and Roles service not available for this launch".to_string())
  })?;
  ...
  let token_response =
    request_service_token_cached(&claims.client_id, &token_url, scopes, &kid, rsa_key)
      .await
  ...
  T: LtiDependencies + Send + Sync + 'static,
{
  ...
  let response = client
    ...
    .header(
      header::ACCEPT,
      "application/vnd.ims.lti-nrps.v2.context+json",
    )
    .send()
    .await

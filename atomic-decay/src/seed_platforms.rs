...
pub async fn seed_platforms(pool: &PgPool) -> Result<(), Box<dyn std::error::Error>> {
...
mod tests {
  ...
  async fn test_seed_platforms() {
    ...
    // Clean up any existing platforms to ensure test isolation
    sqlx::query("TRUNCATE TABLE lti_platforms CASCADE")
      .execute(&pool)
      .await
      .ok();

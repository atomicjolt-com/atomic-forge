#!/bin/bash
# Reset script for Atomic Oxide test database between test runs
...
echo -e "${YELLOW}Resetting Atomic Oxide test database...${NC}"
...
docker exec atomic-forge-postgres psql -U postgres -d atomic_oxide_test -c "
DO \$\$ 
DECLARE
    r RECORD;
BEGIN
